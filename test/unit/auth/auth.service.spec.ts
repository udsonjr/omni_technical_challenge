...
describe('AuthService', () => {
  ...
  describe('refreshToken', () => {
    it('deve retornar newToken quando o token é renovado com sucesso', () => {
      ...
      authRepository.refreshToken.mockReturnValue(
        mockAuthToken({ token: expectedToken, expiresAt: new Date(Date.now() + 1000 * 60 * 60) }),
      );
    ...
    it('deve lançar BadRequestException se token estiver vazio', () => {
      expect(() => service.refreshToken('')).toThrow(BadRequestException);
    });
    ...
    it('deve lançar UnauthorizedException se token for inválido ou expirado', () => {
      ...
      expect(() => service.refreshToken('token-invalido')).toThrow(UnauthorizedException);
    });
  });
  ...
  describe('logout', () => {
    ...
    it('deve lançar BadRequestException se token estiver vazio', () => {
      expect(() => service.logout('')).toThrow(BadRequestException);
    });
